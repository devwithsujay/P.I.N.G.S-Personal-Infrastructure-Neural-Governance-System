...
# ---------------------------------------------------------------------------
# Skip-if-empty enforcement (post-synthesis)
# ---------------------------------------------------------------------------
    ...
    for line in lines:
        ...
        if header_match:
            ...
            if "[SKIP IF" in header_text.upper():
                base_name = re.sub(r"\s*\[SKIP IF.*\]", "", header_text, flags=re.IGNORECASE).strip()
            else:
        ...
        if header_match:
            header_text_full = header_match.group(2).strip()
            if "[SKIP IF" in header_text_full.upper():
                upcoming_content: List[str] = []
            else:
                upcoming_content = []

        if header_match and "[SKIP IF" in header_text_full.upper():
            upcoming_content = []
            continue

        if not header_match and "upcoming_content" in dir() and not skip_until_next_header:
            upcoming_content.append(line)
...
# ---------------------------------------------------------------------------
# Decompose: generate subtopics + classify mode (pure Python, no NIM call)
# ---------------------------------------------------------------------------
